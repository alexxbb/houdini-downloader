...
def download():
    ...
    client_id = "5cKCJj1l7jxk9s0pTUBeNymQLQk3LgTBh6LJRunL"
    client_secret_key = "UlP3oQMHxvinsnbVC5gtWowWeor63WR4pmsF6XqQjYmIy69ciK7oil0rJJc6UYIuqf4MNcyQU2eC6uFrd9UgSizdHHLPygjhBiPUYlbZU2lrxzmCizenKyU4paIkcTem"
    ...
    builds = _service.download.get_daily_builds_list(
        product='houdini', version='19.5', platform='linux', only_production=True)

    import pprint
    print(pprint.pprint(builds[0]))

    return
    # # Retrieve the latest daily build available
    latest_release = _service.download.get_daily_build_download(
        product='houdini', version='19.5', build="production", platform='linux')

    print(latest_release)
    return

    local_filename = Path(args.output_dir, latest_release['filename'])

    # # Download the file
    print(f"Downloading houdini to {local_filename}")
    sys.stdout.flush()
    ...
    if r.status_code == 200:
        ...
        raise Exception(f"Error downloading file, status code: {r.status_code}")
